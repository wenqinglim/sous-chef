...
| `/api/recipes/[id]` | PATCH | `{ status?, tags? }` (at least one) | `{ recipe }` | Set curation status (`tried_and_tested` \| `saved_for_later`) and/or freeform tags; deliberately does **not** flip `edited`. |
